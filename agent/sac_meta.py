...
class MetaSACAgent(Agent):
    ...
    def __init__(self, obs_dim, action_dim, action_range, device, critic_cfg,
        ...
        super().__init__()
        print(obs_dim, action_dim, action_range, device, critic_cfg,
                 actor_cfg, discount, init_temperature, alpha_lr, alpha_betas,
                 actor_lr, actor_betas, actor_update_frequency, critic_lr,
                 critic_betas, critic_tau, critic_target_update_frequency,
                 batch_size, learnable_temperature,
                 normalize_state_entropy)
    ...
    def update(self, replay_buffer: ReplayBuffer, logger, step, gradient_update=1, reward_model=None):
        obs, action, next_obs, not_done, not_done_no_max = replay_buffer.sample(self.batch_size)

        inputs = np.concatenate([obs, action], axis=-1)
        reward = reward_model.r_hat_batch_grad(inputs)

        logger.log('train/batch_reward', reward.detach().cpu().numpy().mean(), step)
    ...
    def update_after_reset(self, replay_buffer: ReplayBuffer, logger, step, gradient_update=1, policy_update=True,
                           reward_model=None):
        for index in range(gradient_update):
            obs, action, next_obs, not_done, not_done_no_max = replay_buffer.sample(self.batch_size)

            inputs = np.concatenate([obs.cpu(), action.cpu()], axis=-1)
            reward = reward_model.r_hat_batch(inputs)
    ...
    def update_state_ent(self, replay_buffer: ReplayBuffer, logger, step, gradient_update=1, K=5, reward_model=None):
        obs, full_obs, action, next_obs, not_done, not_done_no_max = replay_buffer.sample_state_ent(self.batch_size)
