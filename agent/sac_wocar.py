...
class SACAgent(Agent):
    ...
    def get_state_kl_bound_sgld(self, obs, old_dist, eps=0.01, steps=10):
        batch_action_means, stdev = old_dist.mu.detach(), old_dist.std
        states_ub = obs + eps
        states_lb = obs - eps
        step_eps = eps / steps
        ...
        var_states = (obs.clone() + noise.sign() * step_eps).detach().requires_grad_()
        for i in range(steps):
            # Find a nearby state new_phi that maximize the difference
            diff = (self.actor(var_states).mu - batch_action_means) / stdev.detach()
        ...
        diff = (self.actor(var_states.requires_grad_(False)).mu - batch_action_means) / stdev
        return (diff * diff).sum(axis=-1, keepdim=True)
    
    def update_actor_and_alpha(self, obs, intention, logger, step, print_flag=False):
        # policy original loss
        dist = self.actor(obs)
        action = dist.rsample()
        ...
        # attacker Q loss: aim to minimize the attacker's Q value
        worst_Q1, worst_Q2  = intention.critic_target(obs, action)
        worst_Q = torch.min(worst_Q1, worst_Q2)
        q_loss = worst_Q.mean()
        
        # regularize the policy
        # kl_upper_bound = self.get_state_kl_bound_sgld(obs, dist)
        kl_upper_bound = 0
        
        actor_loss_ = actor_loss + 0.5 * q_loss + 0.1 * kl_upper_bound
        ...
        if print_flag:
            ...
            wandb.log({'train_actor/loss': actor_loss, 
                       'train_actor/q_loss': q_loss, 
                       'train_actor/target_entropy': self.target_entropy,
                       'train_actor/entropy': -log_prob.mean()}, step=step)
        ...
        if self.learnable_temperature:
            ...
            if print_flag:
                ...
                wandb.log({'train_alpha/loss': alpha_loss, 'train_alpha/value': self.alpha}, step=step)
            alpha_loss.backward()
            self.log_alpha_optimizer.step()

    def update(self, replay_buffer, intention, logger, step, gradient_update=1):
        for index in range(gradient_update):  # gradient_update = 1
            ...
            if step % self.actor_update_frequency == 0:  # actor_update_frequency = 1
                self.update_actor_and_alpha(obs, intention, logger, step, print_flag)
    ...
    def update_critic_state_ent(self, obs, full_obs, action, next_obs, not_done, logger, step, K=5, print_flag=True):
        ...
        if print_flag:
            ...
            wandb.log({'train_critic/entropy': state_entropy.mean(),
                       'train_critic/entropy_max': state_entropy.max(),
                       'train_critic/entropy_min': state_entropy.min()}, step=step)
        ...
        if print_flag:
            ...
            wandb.log({'train_critic/norm_entropy': norm_state_entropy.mean(),
                       'train_critic/norm_entropy_max': norm_state_entropy.max(),
                       'train_critic/norm_entropy_min': norm_state_entropy.min()}, step=step)
    ...
    def update_after_reset(self, replay_buffer, logger, step, gradient_update=1, policy_update=True):
        for index in range(gradient_update):  # gradient_update = 100
            obs, action, reward, next_obs, not_done, not_done_no_max = replay_buffer.sample(self.batch_size)
            ...
            if index % self.actor_update_frequency == 0 and policy_update:  # actor_update_frequency = 1
                self.update_actor_and_alpha(obs, logger, step, print_flag)

            if index % self.critic_target_update_frequency == 0:  # critic_target_update_frequency = 2
                utils.soft_update_params(self.critic, self.critic_target, self.critic_tau)
