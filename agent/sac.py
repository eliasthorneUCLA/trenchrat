...
class SACAgent(Agent):
    ...
    def __init__(self, obs_dim, action_dim, action_range, device, critic_cfg,
        ...
        self.critic_old_optimizer = torch.optim.Adam(self.critic_old.parameters(), lr=critic_lr, betas=critic_betas)
    ...
    def update_actor_and_alpha(self, obs, logger, step, print_flag=False):
        ...
        if print_flag:
            ...
            wandb.log({'train_actor/loss': actor_loss, 'train_actor/target_entropy': self.target_entropy,
    ...
    def update(self, replay_buffer, logger, step, gradient_update=1, print_flag=True):
        for index in range(gradient_update):  # gradient_update = 1
            ...
            if index == gradient_update - 1 and print_flag:
    ...
    def update_after_reset(self, replay_buffer, logger, step, gradient_update=1, policy_update=True, print_flag=True):
    ...
    def update_state_ent(self, replay_buffer, logger, step, gradient_update=1, K=5, print_flag=True):
        for index in range(gradient_update):  # gradient_update = 1
            ...
            self.update_critic_state_ent(obs, full_obs, action, next_obs, not_done_no_max, logger, step, K=K, print_flag=print_flag)
